...
/**
 * @component Handles displaying and gathering invoice from the server and displaying them to the user.
...
export function InvoiceViewer({ endPoint, updateIsLoadingBoolean, fetchKey }) {
  // Pulls the url queries injected within the url.
  const [queryParameter, setQueryParameters] = useSearchParams();
  const [invoicePath, setInvoicePath] = useState("");

  // Get request made to the server to get the current invoice in pdf form.
  const { isLoading, errorOccurred, fetchData } = UseFetchGetRequest({
    fetchURL: `${endPoint}/get-invoice`,
    key: fetchKey,
  });
  ...
  useEffect(() => {
    ...
    if (errorOccurred) {
      ...
      queryParameter.get("currentInvoice") !== fetchData.fileName ||
      invoicePath === ""
    ) {
  ...
  return (
    <div
      className={`h-100 w-100 ps-mobileLandscape-1 ps-tabletPortrait-0 ps-tabletLandscape-1 pt-1 pt-mobileLandscape-0 pt-tabletPortrait-1 pt-tabletLandscape-0`}
    >
      {isLoading && <h2 className="h-25">Retrieving Invoice</h2>}
      {errorOccurred && <h2 className="h-25">{errorOccurred}</h2>}
      <iframe
        className={`w-100 ${isLoading || errorOccurred ? "h-75" : "h-100"}`}
        src={errorOccurred || isLoading ? "/invalid-invoice.pdf" : invoicePath}
      />
    </div>
  );
}
